<div>
    <x-slot name="titlePage">- Alumnos</x-slot>

    <div class="d-sm-flex justify-content-between align-items-center">
        <div>
            <h1 class="fs-sm-21 fw-700 mb-1">Alumnos</h1>
            <h2 class="fs-sm-14 text-muted">Listado de tus alumnos</h2>
        </div>
        <a class="btn btn-primary py-3 px-4 lh-1 rounded-4">Descargar listado</a>
    </div>
                ...
                <tbody class="fs-sm-14">
                    @foreach ($students as $student)
                        <tr class="border-bottom-dashed align-middle">
                            ...
                            <td><span class="badge fw-500 text-bg-{{ $student -> membership ? $membership_color[$student -> membership -> type] : $membership_color[0] }}">{{ $student -> membership ? $membership_value[$student -> membership -> type] : $membership_value[0] }}</span></td>
                            <td class="text-center">{{ $student -> membership ? $student -> membership -> expiration -> format('d/m/Y') : '-' }}</td>
